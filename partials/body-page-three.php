<div class="bp3" id="cars_for_sale">
    <div class="cars-title">
        <h1 class="title">Used Cars for sales from Our Garage.</h1>
    </div>
    <div class="cars">
        <div class="car-title">
            <h1>Product Title</h1>
            <div id="discount">Discount: 20%</div>
        </div>
        <div class="car-img">
        <img
            src="https://static-eu.cargurus.com/images/forsale/2021/05/12/08/53/2018_kia_soul-pic-2084752386113710083-1024x768.jpeg">
        </div>
        <div class="car-price title">
            <h5>Price: $20.000</h5>
        </div>
    </div>
    <div class="cars">
        <div class="car-title">
            <h1>Product Title</h1>
        </div>
        <div class="car-img">
            <img
            src="https://www.exchangeandmart.co.uk/i/hp/syc.jpg">
        </div>
        <div class="car-price title">
            <h5>Price: $20.000</h5>
        </div>
    </div>
    <div class="cars">
        <div class="car-title">
            <h1>Product Title</h1>
        </div>
        <div class="car-img">
            <img
            src="https://cdn1.buyacar.co.uk/sites/buyacar/files/styles/w860/public/range-rover-evoque-1.jpg?itok=Ks7kn0hf">
        </div>
        <div class="car-price title">
            <h5>Price: $20.000</h5>
        </div>
    </div>
    <div class="cars">
        <div class="car-title">
            <h1>Product Title</h1>
            <div id="discount">Discount: 20%</div>
        </div>
        <div class="car-img">
            <img
            src="https://static-eu.cargurus.com/images/forsale/2021/05/04/22/07/2020_ford_kuga-pic-7219729575545072983-1024x768.jpeg">
        </div>
        <div class="car-price title">
            <h5>Price: $20.000</h5>
        </div>
    </div>
    <!-- </div> -->

</div>